...
router.post('/signup', async (req, res) => {
  ...
  router.post("/chat/:roomId",authenticateJwt, async(req,res)=>{
    ...
    router.post('/upload/:courseId',authenticateJwt,upload.single('video'),async (req, res) => {
      try {
        ...
        if (!course) {
          ...
          let Url = videoFile.path; // Store the path to the uploaded file
          Url = Url.replace(/\\/g, '/'); 
        
          const completeVideoUrl = `http://localhost:3000/${Url}`;
